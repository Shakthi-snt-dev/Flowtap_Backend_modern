...
public class CreateSaleCommandHandler(IApplicationDbContext db, IDateTimeService dateTime)
    : IRequestHandler<CreateSaleCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateSaleCommand request, CancellationToken ct)
    {
        ...
        decimal ticketAdvance = 0;
        string? ticketNumber  = null;
        if (request.TicketId.HasValue)
        {
            var ticketInfo = await db.ServiceTickets
                .Where(t => t.Id == request.TicketId.Value && t.CompanyId == request.CompanyId)
                .Select(t => new { t.Financials!.Prepayment, t.TicketNumber })
                .FirstOrDefaultAsync(ct);

            if (ticketInfo is not null)
            {
                ticketAdvance = ticketInfo.Prepayment;
        ...
        // ── Link ticket → sale and mark paid (if ticket-based sale) ──────────
        if (request.TicketId.HasValue)
        {
            var ticket = await db.ServiceTickets
                .FirstOrDefaultAsync(t => t.Id == request.TicketId.Value && t.CompanyId == request.CompanyId, ct);

            if (ticket is not null)
            {
                ticket.SaleId = sale.Id;
                if (sale.Status == SaleStatus.Completed && ticket.Financials is not null)
