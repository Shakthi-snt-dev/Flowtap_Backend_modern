...
public class GetDeviceModelsQueryHandler(IApplicationDbContext context)
    : IRequestHandler<GetDeviceModelsQuery, Result<List<DeviceModelDto>>>
{
    public async Task<Result<List<DeviceModelDto>>> Handle(GetDeviceModelsQuery request, CancellationToken ct)
    {
        ...
        IQueryable<Flowtap_Domain.BoundedContexts.Modules.Inventory.Entities.DeviceModel> filtered = query;
