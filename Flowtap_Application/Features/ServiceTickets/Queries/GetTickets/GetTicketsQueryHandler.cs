...
public class GetTicketsQueryHandler(IApplicationDbContext db, ICurrentUserService currentUser)
    : IRequestHandler<GetTicketsQuery, Result<PaginatedList<TicketListDto>>>
{
    public async Task<Result<PaginatedList<TicketListDto>>> Handle(GetTicketsQuery request, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Status))
        {
            ...
            {
                query = query.Where(t => t.Status == Flowtap_Domain.BoundedContexts.Modules.ServiceTickets.Enums.TicketStatus.New);
            }
            else if (statusStr.Equals("Completed", StringComparison.OrdinalIgnoreCase))
            {
                query = query.Where(t => t.Status == Flowtap_Domain.BoundedContexts.Modules.ServiceTickets.Enums.TicketStatus.Done);
            }
            else if (statusStr.Equals("Cancelled", StringComparison.OrdinalIgnoreCase))
            {
                query = query.Where(t => t.Status == Flowtap_Domain.BoundedContexts.Modules.ServiceTickets.Enums.TicketStatus.Canceled);
            }
            else
            {
                var normalizedStatus = statusStr.Replace(" ", string.Empty);
                if (Enum.TryParse<Flowtap_Domain.BoundedContexts.Modules.ServiceTickets.Enums.TicketStatus>(normalizedStatus, true, out var statusEnum))
                {
