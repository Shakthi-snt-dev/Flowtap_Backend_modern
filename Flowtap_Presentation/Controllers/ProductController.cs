...
using Flowtap_Application.Features.Inventory.Commands.AddProductDeviceMapping;
using Flowtap_Application.Features.Inventory.Commands.RemoveProductDeviceMapping;
...
using Flowtap_Application.Features.Inventory.Queries.GetProductDeviceMappings;
using MediatR;
...
public class ProductController(ISender sender) : ApiController(sender)
{
    ...
    [HttpGet("{productId:guid}/device-models")]
    public async Task<IActionResult> GetDeviceMappings(Guid productId, CancellationToken ct)
        => Ok(await Sender.Send(new GetProductDeviceMappingsQuery(CurrentTenantId, productId), ct));

    [HttpPost("{productId:guid}/device-models")]
    public async Task<IActionResult> AddDeviceMapping(Guid productId, [FromBody] AddProductDeviceMappingRequest body, CancellationToken ct)
        => Created(await Sender.Send(new AddProductDeviceMappingCommand(CurrentTenantId, productId, body.DeviceModelId), ct));

    [HttpDelete("{productId:guid}/device-models/{deviceModelId:guid}")]
    public async Task<IActionResult> RemoveDeviceMapping(Guid productId, Guid deviceModelId, CancellationToken ct)
        => FromResult(await Sender.Send(new RemoveProductDeviceMappingCommand(CurrentTenantId, productId, deviceModelId), ct));
}

public record AddProductDeviceMappingRequest(Guid DeviceModelId);
